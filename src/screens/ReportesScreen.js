...
export default function ReportesScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      const load = async () => {
        const [a, c, e, l] = await Promise.all([
          getAlumnos(),
          getCursos(),
          getEscuelas(),
          getLocalidades(),
  ...
  const porLocalidad = localidades
    .map((l) => {
      const escIds = escuelas
        .filter((e) => e.localidadId === l.id)
        .map((e) => e.id);
      const curIds = cursos
        .filter((c) => escIds.includes(c.escuelaId))
        .map((c) => c.id);
  ...
  const promAlumnosCurso =
    cursos.length > 0 ? Math.round(alumnos.length / cursos.length) : 0;
